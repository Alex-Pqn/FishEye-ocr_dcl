...
function photographerNotFound() {
  ...
  const mediasSectionElement = document.querySelector(
    '.photograph-medias_section'
  )
  ...
  const filterSectionElement = document.querySelector(
    '.photograph-medias-filters'
  )
  ...
  mainElement.insertAdjacentHTML(
    'beforeend',
    "<p>La page de ce photographe n'existe pas ou a été supprimée.</p>"
  )
}
...
function displayPhotographerHeader() {
  ...
  const contactModalHeaderElement = document.querySelector(
    '#contact_modal header h1 span'
  )
...
function displayPhotographerMedias() {
  const photographerMediasSection = document.querySelector(
    '.photograph-medias_section'
  )
  ...
  // reset medias
  document
    .querySelectorAll('.photograph-media-item')
  ...
  if (isTherePhotographerMedias()) {
      ...
      fetch(newMedia.mediaUrl)
        .then((result) => {
          if (result.ok) {
            ...
            photographerMediasSection.insertAdjacentHTML(
              'beforeend',
              photographerMediaCardDOM
            )
          }
        })
    ...
    photographerMediasSection.insertAdjacentHTML(
      'beforeend',
      `<p>${photographer.name} n'a publié aucun média.</p>`
    )
}
